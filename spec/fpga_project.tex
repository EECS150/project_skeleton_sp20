...
From the previous lab and homework execises, you already had some practice on mapping an algorithmic description of a problem (e.g., loop form) into a circuit implementation. This checkpoint brings it to another level by adding more complexity in terms of the number of loop levels and the amount of operations needed to carry out per loop iteration. How do we make sense between a software loop iteration and a hardware cycle? Would it be possible to map one or more software loop iterations to one hardware cycle? 
...
\item \textbf{req\_read\_len}: Memory read burst length
...
\item \textbf{req\_write\_len}: Memory write burst length
...
\subsection{IO Memory-mapped addresses for control, status, and scalar signals of conv2D accelerator}
...
\begin{table}[hbt]
  \begin{center}
    ...
    \begin{tabular}{l l l l}
      ...
      \verb|32'h80000048| & conv2D feature map dimension & Write & data[31:0] \\
      \verb|32'h8000004c| & conv2D weight offset & Write & data[31:0] \\
      \verb|32'h80000050| & conv2D input feature map offset & Write & data[31:0] \\
      \verb|32'h80000054| & conv2D output feature map offset & Write & data[31:0] \\
      \bottomrule
    \end{tabular}
    \end{adjustbox}
  \end{center}
